...
# Fine-tune model with uploaded images
def fine_tune_model(image_path1, image_path2, vae, num_epochs=300, learning_rate=1e-4):
    ...
    # Use both images to compute the loss and update the model
    ...
    for epoch in range(num_epochs):
        ...
        merged_z = (z1 + z2) / 2  # Merge latents (or use alpha for weighted blending)
        recon_img = vae.decode(merged_z)

        # Loss function (reconstruction loss and KL divergence)
        recon_loss = nn.MSELoss()(recon_img, img_tensor1)  # Reconstruction loss (example)
    ...
    }, 'vae_finetuned2.ckpt')  # Save the updated weights
...
def main():
    ...
    if uploaded_image1 is None or uploaded_image2 is None:
        ...
        if fine_tune_button:
            ...
            # Fine-tune and merge
            fine_tune_model(uploaded_image1, uploaded_image2, vae)

            # Merge and reconstruct after fine-tuning
